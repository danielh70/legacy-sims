...
// =====================
let runMatchPacked = runMatchPackedJS; // JS fallback always exists
let WASM = null; // { exp, hasSeed }
let WASM_READY = false;

// Optional: allow deterministic seeding to also seed WASM
function seedWasmFromS(s) {
  if (!WASM_READY || !WASM || !WASM.hasSeed) return;
  const a = s >>> 0;
  const b = (s ^ 0xa341316c) >>> 0;
  const c = (s ^ 0xc8013ea4) >>> 0;
  const d = (s ^ 0xad90777d) >>> 0;
  WASM.exp.seedRng(a, b, c, d);
}

async function loadWASM() {
  try {
    const path = './combat.wasm';
    console.log(`[DEBUG] Looking for WASM at: ${path}`);
    if (!fs.existsSync(path)) {
      console.log('❌ combat.wasm not found (JS fallback active)');
      return false;
    }

    const wasmBuffer = fs.readFileSync(path);
    console.log(`✅ Found combat.wasm (${wasmBuffer.length} bytes)`);
    ...
    if (typeof exp.runMatchPackedWASM !== 'function') {
      console.log('❌ WASM missing export: runMatchPackedWASM (JS fallback active)');
      return false;
    }

    const hasSeed = typeof exp.seedRng === 'function';
    WASM = { exp, hasSeed };
    WASM_READY = true;

    // Seed once at load (non-deterministic mode uses rolling state)
    if (hasSeed) exp.seedRng(0x9e3779b9, 0x243f6a88, 0xb7e15162, 0xdeadbeef);

    // Wrap: WASM returns i64 -> Node BigInt. Convert to [wins, exSum]
    runMatchPacked = (p1, p2, trials, maxTurns) => {
      const packed = exp.runMatchPackedWASM(
        p1.hp,
        p1.speed,
        p1.acc,
        p1.dodge,
      ...
      // i64 from WASM => BigInt in Node
      const wins = Number((packed >> 32n) & 0xffffffffn);
      const exSum = Number(packed & 0xffffffffn);
      return [wins, exSum];
    };
...
// Original JS version (always available as fallback)
function runMatchPackedJS(p1, p2, trials, MAX_TURNS) {
  let wins = 0;
  let exSum = 0;
...
function printSearchSpaceSanity(
  ...
  console.log(` Effective total: AV * perArmor = ${eff.AV} * ${eff.effPerArmor} = ${eff.effTotal}`);
...
async function workerMain() {
  ...
  // =====================
  // ATTACKER CACHE (v2.13.0)
  // =====================
  const attackerCache = new Map();
  function getCachedAttacker(plan, av, w1v, w2v, m1v, m2v) {
    const key = `${plan.hp}|${av.itemName}|${av.crystalName}|${w1v.itemName}|${w1v.crystalName}|${w1v.upgrade1 || ''}|${w1v.upgrade2 || ''}|${w2v.itemName}|${w2v.crystalName}|${w2v.upgrade1 || ''}|${w2v.upgrade2 || ''}|${m1v.itemName}|${m1v.crystalName}|${m2v.itemName}|${m2v.crystalName}`;
    if (attackerCache.has(key)) return attackerCache.get(key);
    const att = compileAttacker(plan, av, w1v, w2v, m1v, m2v);
    attackerCache.set(key, att);
    return att;
  }
