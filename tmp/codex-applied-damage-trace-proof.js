...
const truthFiles = [
  ['./tmp/legacy-truth-v4-custom-cstaff-full15-merged.json', 'CUSTOM_CSTAFF_A4', 'DL Dual Rift Bio'],
  ['./legacy-truth-v4-custom-maul-a4-dl-abyss-full15.json', 'CUSTOM_MAUL_A4_DL_ABYSS', 'DL Core/Rift Bio'],
  ...
  const sandbox = {
    ...
    USER_CONFIG: {
      attacker: {
        ...
        weapon2: {
          name: 'Crystal Maul',
          upgrades: ['Amulet Crystal', 'Amulet Crystal', 'Amulet Crystal', 'Perfect Fire Crystal'],
        },
        misc1: {
...
function buildCfg(api) {
  ...
  return {
    ...
    diagW2AfterAppliedW1: v.diagW2AfterAppliedW1 || 'auto',
    diagQueuedSecondAction: v.diagQueuedSecondAction || 'auto',
    diagSplitMultiweaponAction: v.diagSplitMultiweaponAction || 'auto',
  };
}
...
function loadTruthBuild(truthPath, attackerLabel, defenderName) {
  ...
  const rows = Array.isArray(raw.matchups)
    ? raw.matchups
    : Array.isArray(raw.rows)
      ? raw.rows
      : Array.isArray(raw.defenders)
        ? raw.defenders
        : [];
  ...
  const defenderRow =
    ...
    rows.find((r) => r.defenderName === defenderName);
...
function parseTraceLine(line, compiled, cfg) {
  ...
  while ((m = weaponRe.exec(rest))) {
    ...
    const actorCompiled = actorSide === 'A' ? compiled.attacker : compiled.defender;
    const targetCompiled = actorSide === 'A' ? compiled.defender : compiled.attacker;
    const weaponCompiled = slot === 1 ? actorCompiled.weapon1 : actorCompiled.weapon2;
    const hpAfter = hp === null ? null : Math.max(0, hp - applied);
    const expectedPostArmor = api.applyArmorAndRound(
      raw,
      targetCompiled.effective.armorFactor,
      cfg.armorRound,
    );
    const stopOnKill =
    ...
    out.push({
      ...
      phase: phase || 'lead',
      weaponSlot: slot,
      weaponName: weaponCompiled.name || (slot === 1 ? 'weapon1' : 'weapon2'),
      compiledRange: weaponCompiled.compiledActionRange,
      rawDamage: raw,
      ...
      targetArmor: targetCompiled.effective.armor,
      targetArmorFactor: targetCompiled.effective.armorFactor,
      armorApply: cfg.armorApply,
      armorRound: cfg.armorRound,
      stopOnKillTriggered: stopOnKill,
      followupPhase: phase,
    });
    hp = hpAfter;
  }
...
const baseCfg = buildCfg(api);

const results = [];

for (const [truthPath, attackerLabel, defenderName] of truthFiles) {
  const { attacker, defender } = loadTruthBuild(truthPath, attackerLabel, defenderName);
  const compiledAttacker = compileBuild(api, attacker, 'A', baseCfg, 'Attacker');
  const compiledDefender = compileBuild(api, defender, 'D', baseCfg, defenderName);
  const compiled = {
    attacker: api.buildCompiledCombatSnapshot(compiledAttacker, compiledDefender, baseCfg, 'floor'),
    defender: api.buildCompiledCombatSnapshot(compiledDefender, compiledAttacker, baseCfg, 'floor'),
  };

  const seed = {
    CUSTOM_CSTAFF_A4: 1337,
    CUSTOM_MAUL_A4_DL_ABYSS: 2337,
    CUSTOM_MAUL_A4_SG1_PINK: defenderName === 'SG1 Double Maul Droid' ? 3337 : 4337,
  }[attackerLabel];
  ...
  const run = api.runMatch(compiledAttacker, compiledDefender, baseCfg, traceCfg);
  const traceEvents = [];
  for (const line of run.traceLines || []) {
    traceEvents.push(...parseTraceLine(line, compiled, baseCfg));
    if (traceEvents.length >= TRACE_LIMIT) break;
  }
  ...
  results.push({
    ...
    rollDumpLines: traceCfg.rollDump.lines || [],
    successfulEvents: traceEvents.slice(0, TRACE_LIMIT),
  });
}

console.log(JSON.stringify({ cfg: baseCfg, results }, null, 2));
