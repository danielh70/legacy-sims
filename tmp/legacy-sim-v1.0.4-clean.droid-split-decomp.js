...
const __DEFAULT_ENV__ = {
  ...
  LEGACY_DIAG_DROID_SPLIT_DECOMP: 'off',
...
function normalizeDiagDroidSplitDecomp(v) {
...
const DIAG_DROID_SPLIT_DECOMP = normalizeDiagDroidSplitDecomp(
  pickEnv('LEGACY_DIAG_DROID_SPLIT_DECOMP', 'off'),
);
...
function doAction(
  ...
  const droidSplitDecompMode =
    ...
    !!att.w2 &&
    att.name !== 'Attacker' &&
    cfg.diagDroidSplitDecomp &&
    cfg.diagDroidSplitDecomp !== 'off'
      ? cfg.diagDroidSplitDecomp
      : 'off';
  ...
  if (splitMultiweaponActionEnabled || droidSplitDecompMode !== 'off') {
    ...
    const gateAfterApplied =
      splitMultiweaponActionEnabled ||
      droidSplitDecompMode === 'gate_only' ||
      droidSplitDecompMode === 'gate_refresh_hit' ||
      droidSplitDecompMode === 'full_split';

    function preForW2() {
      if (splitMultiweaponActionEnabled || droidSplitDecompMode === 'full_split')
        return makePreActionState();
      if (droidSplitDecompMode === 'refresh_pre_only') return makePreActionState();
      if (droidSplitDecompMode === 'refresh_hit_only' || droidSplitDecompMode === 'gate_refresh_hit') {
        const nextPre = makePreActionState();
        return {
          ...pre,
          forceHit: nextPre.forceHit,
        };
      }
      if (droidSplitDecompMode === 'refresh_skill_only') {
        const nextPre = makePreActionState();
        return {
          ...pre,
          sharedSkillFn: nextPre.sharedSkillFn,
          sharedSkillSkillCode: nextPre.sharedSkillSkillCode,
        };
      }
      return pre;
    }

    if (gateAfterApplied && postW1Hp <= 0) {
...
function main() {
  ...
  for (let vi = 0; vi < variants.length; vi++) {
    ...
    const cfg = {
      ...
      diagDroidSplitDecomp: DIAG_DROID_SPLIT_DECOMP,
    ...
    const cfgSigObj = {
      ...
      diagDroidSplitDecomp: cfg.diagDroidSplitDecomp,
