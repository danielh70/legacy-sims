...
function resolveFirstActor(attacker, defender, speedTieMode, diagFirstActorOverride, fightIndex = 0) {
...
function stabilizeCompareStyleStatRoundInput(x, mode, stableCompareStyle) {
  if (!stableCompareStyle || !Number.isFinite(x)) return x;
  if (mode === 'ceil') return x - 1e-9;
  if (mode === 'floor') return x + 1e-9;
  return x;
}
...
function applyCrystalPctToStat(base, pctPerCrystal, n, roundMode, stackMode, stableCompareStyle) {
  base = Number(base) || 0;
  ...
  return (
    x +
    roundStat(
      stabilizeCompareStyleStatRoundInput(x * (pctPerCrystal * n), roundMode, stableCompareStyle),
      roundMode,
    )
  );
}
...
function computeVariant(itemName, crystalName, upgrades = [], cfg, slotTag = 0) {
  ...
  const stackModeStats = cfg.crystalStackStats || 'sum4';
  const stableCompareStyle = !!(cfg && cfg.stableCompareStatRounding);
    ...
    const nLocal =
      crystalKey === 'armor'
        ? Number.isFinite(Number(cfg.armorStatSlots))
          ...
          : nCrystals
        : nCrystals;
    let x = applyCrystalPctToStat(
      base || 0,
      crystalPct[crystalKey] || 0,
      nLocal,
      rMode,
      sMode,
      stableCompareStyle,
...
function applyMixedCrystalPctToStat(
  ...
  return pctSum
    ? base +
        roundStat(
          stabilizeCompareStyleStatRoundInput(base * pctSum, roundMode, stableCompareStyle),
          roundMode,
        )
    : base;
}
...
const __runtimeGateInspectState = {
  firstAnyLogged: false,
  firstAttackerLogged: false,
};

function appendRuntimeGateInspect(tag, att, def, weapon, cfg, weaponSlot) {
  const file = String(process.env.LEGACY_RUNTIME_GATE_LOG || '').trim();
  if (!file) return;
  const actorSide = att && att.name === 'Attacker' ? 'A' : 'D';
  if (tag === 'first_any' && __runtimeGateInspectState.firstAnyLogged) return;
  if (tag === 'first_attacker' && __runtimeGateInspectState.firstAttackerLogged) return;
  const payload = {
    tag,
    pid: process.pid,
    verifyDefenders: String(process.env.LEGACY_VERIFY_DEFENDERS || ''),
    attackerAttackTypeEnv: String(process.env.LEGACY_ATTACKER_ATTACK_TYPE || ''),
    defenderAttackTypeEnv: String(process.env.LEGACY_DEFENDER_ATTACK_TYPE || ''),
    actorSide,
    weaponSlot,
    weaponName: weapon && weapon.name ? weapon.name : '',
    armorApply: cfg && cfg.armorApply ? cfg.armorApply : '',
    att: att
      ? {
          name: att.name || '',
          speed: att.speed,
          armor: att.armor,
          armorFactor: att.armorFactor,
          acc: att.acc,
          dodge: att.dodge,
          gun: att.gun,
          mel: att.mel,
          prj: att.prj,
          defSk: att.defSk,
          w1: att.w1 ? { name: att.w1.name || '', min: att.w1.min, max: att.w1.max, skill: att.w1.skill } : null,
          w2: att.w2 ? { name: att.w2.name || '', min: att.w2.min, max: att.w2.max, skill: att.w2.skill } : null,
          meta: att.__meta || null,
        }
      : null,
    def: def
      ? {
          ...
          prj: def.prj,
          defSk: def.defSk,
          w1: def.w1 ? { name: def.w1.name || '', min: def.w1.min, max: def.w1.max, skill: def.w1.skill } : null,
          w2: def.w2 ? { name: def.w2.name || '', min: def.w2.min, max: def.w2.max, skill: def.w2.skill } : null,
          meta: def.__meta || null,
        }
      : null,
  };
  fs.appendFileSync(file, `${JSON.stringify(payload)}\n`);
  if (tag === 'first_any') __runtimeGateInspectState.firstAnyLogged = true;
  if (tag === 'first_attacker') __runtimeGateInspectState.firstAttackerLogged = true;
}
...
function attemptWeapon(
  ...
  appendRuntimeGateInspect('first_any', att, def, weapon, cfg, weaponSlot);
  if (att && att.name === 'Attacker') {
    appendRuntimeGateInspect('first_attacker', att, def, weapon, cfg, weaponSlot);
  }
  counters.attempts++;
...
function doAction(
  ...
  if (splitMultiweaponActionEnabled) {
    ...
    if (postW1Hp <= 0) {
      ...
      if (diagSide) diagSide.splitActionKilledAfterW1 = (diagSide.splitActionKilledAfterW1 || 0) + 1;
    } else {
...
function fightOnce(p1, p2, cfg, stats, trace, rollCtx = null, fightIndex = 0) {
  ...
  const p1First = resolveFirstActor(p1, p2, cfg.speedTieMode, cfg.diagFirstActorOverride, fightIndex);
  ...
  while (p1hp > 0 && p2hp > 0 && turns < cfg.maxTurns) {
    ...
    if (cfg.roundResolveMode === 'simultaneous_round') {
      ...
      const hpFirst0 = first === p1 ? (queuedSecondActionEnabled ? p1hp0 : p1hp) : queuedSecondActionEnabled ? p2hp0 : p2hp;
      const r2 = doAction(
        second,
        first,
        hpFirst0,
...
function main() {
  ...
  for (let vi = 0; vi < variants.length; vi++) {
    ...
    const cache = new Map();
    function cfgForRepresentedBuildPart(part, baseCfg) {
      const partSlots = resolvedPartCrystalSlots(part, baseCfg.crystalSlots);
      if (!useRepresentedBuildStatSemantics(part)) {
        return {
          ...baseCfg,
      ...
      // Represented-build stat patch: explicit per-part slot-count builds use compare-style stat stacking.
      return {
        ...baseCfg,
        crystalSlots: partSlots,
    ...
    const vKey = (itemName, crystalSpec, cfgLocal, u1 = '', u2 = '') => {
      const crystalKey = crystalSpec ? crystalSpecKey(crystalSpec, cfgLocal.crystalSlots) : '';
      return [
        cfgLocal.statRound,
    ...
    function getV(itemName, crystalSpec, cfgLocal, u1 = '', u2 = '', slotTag = 0) {
      const k = vKey(itemName, crystalSpec, cfgLocal, u1, u2);
      let vv = cache.get(k);
      if (!vv) {
        const ups = [];
        if (u1) ups.push(u1);
        if (u2) ups.push(u2);
        vv = computeVariantFromCrystalSpec(itemName, crystalSpec, ups, cfgLocal, slotTag);
        cache.set(k, vv);
      }
      return vv;
    }
    ...
    const aArmorCfg = cfgForRepresentedBuildPart(a.armor, cfg);
    const aW1Cfg = cfgForRepresentedBuildPart(a.weapon1, cfg);
    const aW2Cfg = cfgForRepresentedBuildPart(a.weapon2, cfg);
    const aM1Cfg = cfgForRepresentedBuildPart(a.misc1, cfg);
    const aM2Cfg = cfgForRepresentedBuildPart(a.misc2, cfg);
    const attacker = compileCombatantFromParts({
      name: 'Attacker',
      stats: a.stats,
      armorV: getV(a.armor.name, partCrystalSpec(a.armor), aArmorCfg),
      w1V: getV(
        a.weapon1.name,
      ...
      m1V: getV(a.misc1.name, partCrystalSpec(a.misc1), aM1Cfg, '', '', 1),
      m2V: getV(a.misc2.name, partCrystalSpec(a.misc2), aM2Cfg, '', '', 2),
      cfg,
      role: 'A',
    ...
    for (let di = 0; di < payloads.length; di++) {
      ...
      const dArmorCfg = cfgForRepresentedBuildPart(payload.armor, cfg);
      const dW1Cfg = cfgForRepresentedBuildPart(payload.weapon1, cfg);
      const dW2Cfg = cfgForRepresentedBuildPart(payload.weapon2, cfg);
      const dM1Cfg = cfgForRepresentedBuildPart(payload.misc1, cfg);
      const dM2Cfg = cfgForRepresentedBuildPart(payload.misc2, cfg);
      const defender = compileCombatantFromParts({
        name,
        stats: payload.stats,
        armorV: getV(payload.armor.name, partCrystalSpec(payload.armor), dArmorCfg),
        w1V: (() => {
          const [u1, u2] = normalizeResolvedBuildWeaponUpgrades(payload.weapon1);
          return getV(payload.weapon1.name, partCrystalSpec(payload.weapon1), dW1Cfg, u1, u2);
        })(),
        w2V: (() => {
          const [u1, u2] = normalizeResolvedBuildWeaponUpgrades(payload.weapon2);
          return getV(payload.weapon2.name, partCrystalSpec(payload.weapon2), dW2Cfg, u1, u2);
        })(),
        m1V: getV(payload.misc1.name, partCrystalSpec(payload.misc1), dM1Cfg, '', '', 1),
        m2V: getV(payload.misc2.name, partCrystalSpec(payload.misc2), dM2Cfg, '', '', 2),
        cfg,
        role: 'D',
