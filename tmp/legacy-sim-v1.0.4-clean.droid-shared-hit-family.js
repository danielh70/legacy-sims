...
const __DEFAULT_ENV__ = {
  ...
  LEGACY_DIAG_DROID_SHARED_HIT_FAMILY: 'off',
...
function normalizeDiagDroidSharedHitFamily(v) {
  const s = String(v || 'off')
    .trim()
    .toLowerCase();
  return s === 'off' ||
    s === 'global_refresh_hit' ||
    s === 'same_skill_refresh_hit' ||
    s === 'melee_pair_refresh_hit' ||
    s === 'same_name_refresh_hit' ||
    s === 'same_name_gate_hit' ||
    s === 'melee_pair_gate_hit' ||
    s === 'same_skill_gate_hit' ||
    s === 'identical_dual_refresh_hit' ||
...
function diagDroidSharedHitFamilyModeForActor(mode, att, cfg, baseVal) {
  if (
    mode === 'off' ||
    !att ||
    att.name === 'Attacker' ||
  ...
  const w1 = att.w1 || {};
  const w2 = att.w2 || {};
  const sameSkill = w1.skill === w2.skill;
  const meleePair = w1.skill === 1 && w2.skill === 1;
  const sameName = !!sig && !!sig.w1Item && sig.w1Item === sig.w2Item;
  const identicalDual =
    !!sig &&
    sameName &&
    !!sig.w1CrystalSpec &&
    !!sig.w2CrystalSpec &&
    (sig.w1Crystal || '') === (sig.w2Crystal || '') &&
    JSON.stringify(sig.w1CrystalSpec || null) === JSON.stringify(sig.w2CrystalSpec || null) &&
    JSON.stringify(sig.w1Upgrades || []) === JSON.stringify(sig.w2Upgrades || []);
  const exactDroid =
    !!sig &&
    ...
    sig.m2Crystal === 'Perfect Orange Crystal';

  const pred =
    mode === 'global_refresh_hit'
      ? true
      : mode === 'same_skill_refresh_hit' || mode === 'same_skill_gate_hit'
        ? sameSkill
        : mode === 'melee_pair_refresh_hit' || mode === 'melee_pair_gate_hit'
          ? meleePair
          : mode === 'same_name_refresh_hit' || mode === 'same_name_gate_hit'
            ? sameName
            : mode === 'identical_dual_refresh_hit'
              ? identicalDual
  ...
  return {
    enabled: !!pred,
    gate:
      mode === 'same_skill_gate_hit' ||
      mode === 'melee_pair_gate_hit' ||
      mode === 'same_name_gate_hit',
    refreshHit: mode !== 'off' && !!pred,
    label: pred ? mode : 'off',
  };
}
...
const DIAG_DROID_SHARED_HIT_FAMILY = normalizeDiagDroidSharedHitFamily(
  pickEnv('LEGACY_DIAG_DROID_SHARED_HIT_FAMILY', 'off'),
);
...
function doAction(
  ...
  if (!sideStats.runtimeActionAudit) {
    sideStats.runtimeActionAudit = {
      ...
      diagDroidSharedHitFamily: cfg.diagDroidSharedHitFamily || 'off',
    };
  }
  ...
  const droidSharedHitFamilyMode = diagDroidSharedHitFamilyModeForActor(
    cfg.diagDroidSharedHitFamily,
    att,
    cfg,
    baseVal,
  );

  if (splitMultiweaponActionEnabled || droidSharedHitFamilyMode.enabled) {
    ...
    const gateAfterApplied =
      splitMultiweaponActionEnabled || droidSharedHitFamilyMode.gate;
    ...
    if (gateAfterApplied && postW1Hp <= 0) {
      ...
      let preW2 = splitMultiweaponActionEnabled ? makePreActionState() : pre;
      if (!splitMultiweaponActionEnabled && droidSharedHitFamilyMode.refreshHit) {
...
function main() {
  ...
  for (let vi = 0; vi < variants.length; vi++) {
    ...
    const cfg = {
      ...
      diagDroidSharedHitFamily: DIAG_DROID_SHARED_HIT_FAMILY,
    ...
    const cfgSigObj = {
      ...
      diagDroidSharedHitFamily: cfg.diagDroidSharedHitFamily,
