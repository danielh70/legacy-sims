...
const __DEFAULT_ENV__ = {
  ...
  LEGACY_DIAG_DROID_HF_SHELL_MODE: 'off',
...
function normalizeDiagDroidVsHfShellMode(v) {
  const s = String(v || 'off')
    .trim()
    .toLowerCase();
  return s === 'off' ||
    s === 'same_name_refresh_hit' ||
    s === 'same_name_refresh_hit_w1_mixed' ||
    s === 'same_name_refresh_hit_w2_mixed' ||
    s === 'same_name_gate_refresh_hit_w1_mixed' ||
    s === 'same_name_refresh_hit_hf_misc' ||
    s === 'same_name_refresh_hit_droid_misc' ||
    s === 'same_name_refresh_hit_hf_armor' ||
    s === 'same_name_refresh_hit_sg1_armor_droid_misc' ||
    s === 'same_name_refresh_hit_sg1_armor' ||
    s === 'exact_droid_refresh_hit' ||
    s === 'exact_hf_refresh_hit'
    ? s
    : 'off';
}
...
function diagDroidVsHfShellModeForActor(mode, att, cfg, baseVal) {
  if (
    mode === 'off' ||
    !att ||
    att.name === 'Attacker' ||
    ...
    return { enabled: false, gate: false, refreshHit: false, label: 'off' };
  }

  const sig = att.__runtimeSig || null;
  if (!sig) return { enabled: false, gate: false, refreshHit: false, label: 'off' };

  const sameName = !!sig.w1Item && sig.w1Item === sig.w2Item;
  const meleePair = att.w1.skill === 1 && att.w2.skill === 1;
  const w1Weight = Number(att.w1.min || 0) + Number(att.w1.max || 0);
  const w2Weight = Number(att.w2.min || 0) + Number(att.w2.max || 0);
  const w1Mixed = w1Weight > w2Weight;
  const w2Mixed = w2Weight > w1Weight;
  const exactDroid =
    sig.armorItem === 'SG1 Armor' &&
    sig.w1Item === 'Crystal Maul' &&
    sig.w2Item === 'Crystal Maul' &&
    sig.m1Item === 'Scout Drones' &&
    sig.m2Item === 'Droid Drone';
  const exactHf =
    sig.armorItem === 'Hellforged Armor' &&
    sig.w1Item === 'Scythe T2' &&
    sig.w2Item === 'Scythe T2' &&
    sig.m1Item === 'Scout Drones' &&
    sig.m2Item === 'Bio Spinal Enhancer';

  const pred =
    mode === 'same_name_refresh_hit'
      ? sameName && meleePair
      : mode === 'same_name_refresh_hit_w1_mixed' ||
          mode === 'same_name_gate_refresh_hit_w1_mixed'
        ? sameName && meleePair && w1Mixed && !w2Mixed
        : mode === 'same_name_refresh_hit_w2_mixed'
          ? sameName && meleePair && w2Mixed && !w1Mixed
          : mode === 'same_name_refresh_hit_hf_misc'
            ? sameName && meleePair && sig.m2Item === 'Bio Spinal Enhancer'
            : mode === 'same_name_refresh_hit_droid_misc'
              ? sameName && meleePair && sig.m2Item === 'Droid Drone'
              : mode === 'same_name_refresh_hit_hf_armor'
                ? sameName && meleePair && sig.armorItem === 'Hellforged Armor'
                : mode === 'same_name_refresh_hit_sg1_armor_droid_misc'
                  ? sameName &&
                    meleePair &&
                    sig.armorItem === 'SG1 Armor' &&
                    sig.m2Item === 'Droid Drone'
                : mode === 'same_name_refresh_hit_sg1_armor'
                  ? sameName && meleePair && sig.armorItem === 'SG1 Armor'
          : mode === 'exact_droid_refresh_hit'
            ? exactDroid
            : mode === 'exact_hf_refresh_hit'
              ? exactHf
              : false;

  return {
    enabled: !!pred,
    gate: mode === 'same_name_gate_refresh_hit_w1_mixed',
    refreshHit: !!pred,
    label: pred ? mode : 'off',
  };
}
...
const DIAG_DROID_HF_SHELL_MODE = normalizeDiagDroidVsHfShellMode(
  pickEnv('LEGACY_DIAG_DROID_HF_SHELL_MODE', 'off'),
);
...
function compileCombatantFromParts({
  ...
  c.__runtimeSig = {
    ...
    armorCrystal: armorV.crystalName || '',
    armorCrystalSpec: armorV.crystalSpecShort || null,
    w1Item: w1V.itemName || '',
    w1Crystal: w1V.crystalName || '',
    w1CrystalSpec: w1V.crystalSpecShort || null,
    w1Upgrades: Array.isArray(w1V.upgrades) ? w1V.upgrades.slice() : [],
    w2Item: w2V.itemName || '',
    w2Crystal: w2V.crystalName || '',
    w2CrystalSpec: w2V.crystalSpecShort || null,
    w2Upgrades: Array.isArray(w2V.upgrades) ? w2V.upgrades.slice() : [],
    m1Item: m1Eff.itemName || '',
    m1Crystal: m1Eff.crystalName || '',
    m1CrystalSpec: m1Eff.crystalSpecShort || null,
    m2Item: m2Eff.itemName || '',
    m2Crystal: m2Eff.crystalName || '',
    m2CrystalSpec: m2Eff.crystalSpecShort || null,
  };
...
function doAction(
  ...
  const droidVsHfShellMode = diagDroidVsHfShellModeForActor(
    cfg.diagDroidVsHfShellMode,
    att,
    cfg,
    baseVal,
  );
  ...
  if (!sideStats.runtimeActionAudit) {
    sideStats.runtimeActionAudit = {
      ...
      diagDroidVsHfShellMode: cfg.diagDroidVsHfShellMode || 'off',
    };
  }
  ...
  if (splitMultiweaponActionEnabled || droidVsHfShellMode.enabled) {
    ...
    if (postW1Hp <= 0 || (droidVsHfShellMode.gate && w1AppliedSplit >= targetHp0)) {
      if (traceW)
        traceW.push({
          h: false,
          s: false,
          ...
          d: 0,
          skip: true,
          diag: droidVsHfShellMode.enabled ? droidVsHfShellMode.label : 'split',
        });
      ...
      const w2Pre =
        droidVsHfShellMode.refreshHit && !splitMultiweaponActionEnabled ? makePreActionState() : makePreActionState();
      r2 = attemptWeapon(
        att,
        def,
        att.w2,
...
function main() {
  ...
  for (let vi = 0; vi < variants.length; vi++) {
    ...
    const cfg = {
      ...
      diagDroidVsHfShellMode: DIAG_DROID_HF_SHELL_MODE,
    ...
    const cfgSigObj = {
      ...
      diagDroidVsHfShellMode: cfg.diagDroidVsHfShellMode,
        ...
        ? {
            ...
            critical: firstVariant.config
              ? {
                  ...
                  diagDroidVsHfShellMode: firstVariant.config.diagDroidVsHfShellMode,
