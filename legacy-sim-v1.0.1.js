...
function loadBaselineFile(filePath) {
  try {
    ...
    console.error(
      `NOTE: failed to parse baselines file "${filePath}": ${e.message}`,
    );
    return null;
  }
}
...
function baselineLookup(store, keyHex, defenderName) {
  ...
  if (r && r.defenders && r.defenders[defenderName])
    return r.defenders[defenderName];
...
const MISC_NO_CRYSTAL_SKILL_TWEAK_TAG = (() => {
  if (!MISC_NO_CRYSTAL_SKILL.size || !MISC_NO_CRYSTAL_SKILL_TYPE_MULTS.size)
    return "none";
...
const SWAP_ATTACKER_WEPS = yn(
  String(pickEnv("LEGACY_SWAP_ATTACKER_WEPS", "0")),
);
const SWAP_DEFENDER_WEPS = yn(
  String(pickEnv("LEGACY_SWAP_DEFENDER_WEPS", "0")),
);
...
function rollVsDump(
  ...
  if (ctx && ctx.enabled && meta) {
    ...
    const offNote =
      q !== "none"
        ? `${fmtBound(aMinRaw)}→${fmtBound(aMin)}`
        : fmtBound(aMinRaw);
    const defNote =
      q !== "none"
        ? `${fmtBound(dMinRaw)}→${fmtBound(dMin)}`
        : fmtBound(dMinRaw);
...
function probUniformGreaterFloat(a0, a1, b0, b1) {
  ...
  if (seg2_end > seg2_start) {
    ...
    const part =
      (a1 * len - 0.5 * (seg2_end * seg2_end - seg2_start * seg2_start)) / A;
    area += part;
  }
...
function predictMaxActionFromWeaponMax(
  w1Max,
  w2Max,
  level,
  armor,
...
function predictMinActionFromWeaponMin(
  w1Min,
  w2Min,
  level,
  armor,
...
const HF_ARMOR_BASE_OVERRIDE = Number(
  pickEnv("LEGACY_HF_ARMOR_BASE_OVERRIDE", ""),
);
if (Number.isFinite(HF_ARMOR_BASE_OVERRIDE) && HF_ARMOR_BASE_OVERRIDE > 0) {
  if (ItemDefs["Hellforged Armor"] && ItemDefs["Hellforged Armor"].flatStats) {
    ItemDefs["Hellforged Armor"].flatStats.armor = Math.floor(
      HF_ARMOR_BASE_OVERRIDE,
    );
  }
}

const VOID_SWORD_BASE_MIN_OVERRIDE = Number(
  pickEnv("LEGACY_VOID_SWORD_BASE_MIN_OVERRIDE", ""),
);
const VOID_SWORD_BASE_MAX_OVERRIDE = Number(
  pickEnv("LEGACY_VOID_SWORD_BASE_MAX_OVERRIDE", ""),
);
if (ItemDefs["Void Sword"] && ItemDefs["Void Sword"].baseWeaponDamage) {
  if (
    Number.isFinite(VOID_SWORD_BASE_MIN_OVERRIDE) &&
    VOID_SWORD_BASE_MIN_OVERRIDE > 0
  ) {
    ItemDefs["Void Sword"].baseWeaponDamage.min = Math.floor(
      VOID_SWORD_BASE_MIN_OVERRIDE,
    );
  }
  if (
    Number.isFinite(VOID_SWORD_BASE_MAX_OVERRIDE) &&
    VOID_SWORD_BASE_MAX_OVERRIDE > 0
  ) {
    ItemDefs["Void Sword"].baseWeaponDamage.max = Math.floor(
      VOID_SWORD_BASE_MAX_OVERRIDE,
    );
  }
  if (
...
const GAME_TRIALS = Math.max(
  0,
  parseInt(pickEnv("LEGACY_GAME_TRIALS", "10000"), 10) || 0,
);
...
function applyCrystalPctToWeaponDmg(
  ...
  if (stackMode === "iter4") {
    for (let i = 0; i < n; i++)
      x = roundWeaponDmg(x * (1 + pctPerCrystal), weaponRoundMode);
    return x;
  }
...
function computeVariant(
  ...
  if (upgrades && upgrades.length) {
    for (const u of upgrades) {
      ...
      if (!udef)
        throw new Error(`Unknown upgrade "${u}" on item "${itemName}"`);
      const up = udef.pct || {};
      for (const k of Object.keys(up))
        upgradePct[k] = (upgradePct[k] || 0) + (up[k] || 0);
    }
  }
  ...
  function applyStat(base, crystalKey, upgradeKey) {
    const rMode =
      crystalKey === "armor" ? cfg.armorStatRound || statRound : statRound;
    const sMode =
      crystalKey === "armor"
        ? cfg.armorStatStack || stackModeStats
        : stackModeStats;
    const nLocal =
      crystalKey === "armor"
...
function applyHiddenRoleBonuses(c, role, cfg) {
  ...
  if (cfg.hiddenPreset === "slot3") {
    ...
    const prjCount =
      (c.weapon1 && c.weapon1.skill === 2 ? 1 : 0) +
      (c.weapon2 && c.weapon2.skill === 2 ? 1 : 0);
...
if (
  !DEFENDER_PAYLOADS["SG1 Split Bombs T2"] &&
  DEFENDER_PAYLOADS["SG1 Split bombs"]
) {
  DEFENDER_PAYLOADS["SG1 Split Bombs T2"] =
    DEFENDER_PAYLOADS["SG1 Split bombs"];
}
...
if (
  !DEFENDER_PAYLOADS["Dual Bow Build"] &&
  DEFENDER_PAYLOADS["Dual Bow Build 1"]
) {
...
if (
  !DEFENDER_PAYLOADS["Bow/rift build"] &&
  DEFENDER_PAYLOADS["Rift Bow Build 1"]
) {
...
function getBuiltinBaselineDefenderNames(attackerPreset) {
  const out = [];
  const seen = new Set();
  ...
  for (const rawKey of Object.keys(GAME_BASELINES)) {
    ...
    if (key.includes("|")) {
      ...
      if ((parts[0] || "") !== attackerPreset) continue;
...
function resolveVerifySelection(rawCsv, attackerPreset) {
  const tokens = parseCsv(rawCsv);
  const baselineNames = getBuiltinBaselineDefenderNames(attackerPreset);

  if (!tokens.length) {
    const priorityNames = canonicalizeDefenderList(DEFENDER_PRIORITY);
    const priorityWithBaselines = priorityNames.filter((n) =>
      baselineNames.includes(n),
    );
    return {
      names: priorityWithBaselines.length
        ? priorityWithBaselines
        : priorityNames,
      mode: priorityWithBaselines.length
        ? "default:priority+baselines"
        : "default:priority",
      raw: "",
    };
  }
...
function resolveAttackerSelection() {
  const modeRaw =
    (USER_CONFIG && USER_CONFIG.attacker && USER_CONFIG.attacker.mode) || "env";
  const mode = String(modeRaw).trim().toLowerCase();
  const presetFallback =
    (USER_CONFIG && USER_CONFIG.attacker && USER_CONFIG.attacker.preset) ||
    "MAUL_CSTAFF";

  if (mode === "custom") {
    const build =
      USER_CONFIG && USER_CONFIG.attacker && USER_CONFIG.attacker.custom;
  ...
  const raw =
    mode === "preset"
      ...
      : pickEnv("LEGACY_ATTACKER_PRESET", presetFallback);

  const name = resolveKeyOrFallback(
    raw,
    ATTACKER_PRESETS,
...
function colorFirst(firstDisp) {
  ...
  const c =
    firstDisp === "A" ? ANSI.green : firstDisp === "D" ? ANSI.red : ANSI.yellow;
...
function attemptWeapon(
  ...
  const skillLabel =
    weapon.skill === 0 ? "Gun" : weapon.skill === 1 ? "Mel" : "Prj";

  let s;
  if (
    (cfg.sharedSkillMode === "same_type" ||
      cfg.sharedSkillMode === "gun_same_type") &&
    pre.sharedSkillFn &&
  ...
  const postArmorPerWeapon = applyArmorAndRound(
    raw,
    def.armorFactor,
    cfg.armorRound,
  );
  ...
  if (cfg.armorApply === "per_weapon" && postArmorPerWeapon > 0) {
    if (postArmorPerWeapon < counters.rngMin)
      counters.rngMin = postArmorPerWeapon;
    if (postArmorPerWeapon > counters.rngMax)
      counters.rngMax = postArmorPerWeapon;
  }
...
function doAction(
  ...
  if (att.w1 && att.w2 && att.w1.skill === att.w2.skill) {
    ...
    const allow =
      cfg.sharedSkillMode === "same_type" ||
      (cfg.sharedSkillMode === "gun_same_type" && sc === 0);
  ...
  if (cfg.armorApply === "per_weapon") {
    ...
    if (
      baseVal > 0 &&
      cfg.dmgBonusMode === "per_action" &&
      cfg.dmgBonusStage === "post_armor"
    ) {
...
function fightOnce(p1, p2, cfg, stats, trace, rollCtx = null) {
  ...
  while (p1hp > 0 && p2hp > 0 && turns < cfg.maxTurns) {
    ...
    if (rollCtx) {
      rollCtx.turn = turns;
      if (rollCtx.maxTurns > 0 && turns > rollCtx.maxTurns)
        rollCtx.enabled = false;
    }
...
function runMatch(attacker, defender, cfg, traceCfg) {
  ...
  for (let i = 0; i < cfg.trials; i++) {
    ...
    if (
      ...
      rollDumpPush(
        rollCtx,
        `RD === BEGIN def="${rollCtx.matchName}" fight=${rollCtx.fight} ===`,
      );
    }

    const { winnerIsP1, turns } = fightOnce(
      attacker,
      defender,
      cfg,
      stats,
...
function makeVariantList() {
  ...
  const projDef = listOrSingle(
    "LEGACY_PROJ_DEF_MULT",
    (s) => parseNumCsv(s),
    "1",
  );
  ...
  const hitGe = listOrSingleChained(
    ...
    (s) =>
      parseCsv(s).map(
        (x) =>
          x === "1" || x.toLowerCase() === "true" || x.toLowerCase() === "ge",
      ),
    "0",
    "LEGACY_GE",
  );
  ...
  const crystalSlots = listOrSingle(
    "LEGACY_CRYSTAL_SLOTS",
    (s) => parseNumCsv(s),
    "4",
  );
  ...
  function zipLists() {
    ...
    for (const [k, arr] of Object.entries(lists)) {
      if (arr.length !== 1 && arr.length !== N) {
        throw new Error(
          `ZIP sweep mismatch: ${k} has length ${arr.length} but expected 1 or ${N}`,
        );
      }
    }
    ...
    for (let i = 0; i < N; i++) {
      out.push({
        ...
        skillRollMode:
          skillRollMode.length === 1 ? skillRollMode[0] : skillRollMode[i],
        ...
        dmgBonusMode:
          dmgBonusMode.length === 1 ? dmgBonusMode[0] : dmgBonusMode[i],
        dmgBonusStage:
          dmgBonusStage.length === 1 ? dmgBonusStage[0] : dmgBonusStage[i],
        statRound: statRound.length === 1 ? statRound[0] : statRound[i],
        weaponDmgRound:
          weaponDmgRound.length === 1 ? weaponDmgRound[0] : weaponDmgRound[i],
        armorStatStack:
          armorStatStack.length === 1 ? armorStatStack[0] : armorStatStack[i],
        armorStatRound:
          armorStatRound.length === 1 ? armorStatRound[0] : armorStatRound[i],
        armorStatSlots:
          armorStatSlots.length === 1 ? armorStatSlots[0] : armorStatSlots[i],
        crystalStackStats:
          crystalStackStats.length === 1
            ? crystalStackStats[0]
            : crystalStackStats[i],
        crystalStackDmg:
          crystalStackDmg.length === 1
            ? crystalStackDmg[0]
            : crystalStackDmg[i],
        crystalSlots:
          crystalSlots.length === 1 ? crystalSlots[0] : crystalSlots[i],
        sharedHit: sharedHit.length === 1 ? sharedHit[0] : sharedHit[i],
        sharedSkillMode:
          sharedSkill.length === 1 ? sharedSkill[0] : sharedSkill[i],
        speedTieMode:
          speedTieMode.length === 1 ? speedTieMode[0] : speedTieMode[i],
        hiddenPreset:
          hiddenPreset.length === 1 ? hiddenPreset[0] : hiddenPreset[i],
      });
    }
    return out;
  }
...
function main() {
  const trials = Math.max(
    1,
    parseInt(pickEnv("LEGACY_TRIALS", String(DEFAULTS.TRIALS)), 10),
  );
  ...
  const rollDumpDefEnv = String(
    pickEnv("LEGACY_ROLL_DUMP_DEFENDERS", ""),
  ).trim();
  const rollDumpFights = Math.max(
    0,
    parseInt(pickEnv("LEGACY_ROLL_DUMP_FIGHTS", "1"), 10) || 0,
  );
  ...
  const diagRangeDefFilter = String(
    pickEnv("LEGACY_DIAG_RANGE_DEFENDER", "") || "",
  )
    .trim()
    .toLowerCase();
  ...
  const traceFights = Math.max(
    0,
    parseInt(
      pickEnv("LEGACY_TRACE_FIGHTS", pickEnv("LEGACY_TRACE", "0")),
      10,
    ) || 0,
  );

  const outputMode = String(
    pickEnvAny(["LEGACY_OUTPUT", "LEGACY_REPORT"], "compact"),
  )
    .trim()
    .toLowerCase();
  const outputVerbose =
    outputMode === "verbose" || outputMode === "full" || outputMode === "1";
  const printExact = yn(
    pickEnv("LEGACY_PRINT_EXACT", outputVerbose ? "1" : "0"),
  );
  const printGame = yn(pickEnv("LEGACY_PRINT_GAME", "0"));
  const doctorOn = yn(pickEnv("LEGACY_DOCTOR", "1"));
  const compareTopN = Math.max(
    0,
    parseInt(pickEnv("LEGACY_COMPARE_TOP", "5"), 10) || 5,
  );
  const exportJsonOn = yn(pickEnv("LEGACY_EXPORT_JSON", "0"));
  const exportJsonFileRaw = String(
    pickEnv("LEGACY_EXPORT_JSON_FILE", ""),
  ).trim();
  ...
  const headerMode = String(pickEnv("LEGACY_HEADER", "min"))
    .trim()
    .toLowerCase();
  const defListMax = Math.max(
    0,
    parseInt(pickEnv("LEGACY_DEF_LIST_MAX", "6"), 10) || 6,
  );

  const compareBaselines = yn(pickEnv("LEGACY_COMPARE", "0"));
  const baselinesFile = String(pickEnv("LEGACY_BASELINES_FILE", "")).trim();
  const baselineKeyOverride = String(pickEnv("LEGACY_BASELINE_KEY", ""))
    .trim()
    .toLowerCase();
  const baselineFallback = yn(pickEnv("LEGACY_BASELINE_FALLBACK", "0"));
  const printBaselineTemplate = yn(
    pickEnv("LEGACY_PRINT_BASELINE_TEMPLATE", "0"),
  );
  ...
  if (
    (compareBaselines || printBaselineTemplate) &&
    baselinesFile &&
    !baselineStore
  ) {
    console.error(
  ...
  if (rollDumpOn && rollDumpFights > 0) {
    ...
    rollDumpNameKeys = new Set(
      parseCsv(rollDumpDefRaw).map((x) => normalizeNameKey(x)),
    );
    wantRollDumpFor = (defName) =>
      rollDumpOn &&
      rollDumpFights > 0 &&
      rollDumpNameKeys.has(normalizeNameKey(defName));
  }
  const nameWEnv = Math.max(
    0,
    parseInt(pickEnv("LEGACY_NAME_W", "0"), 10) || 0,
  );
  const maxNameLen = defenderNames.reduce(
    (m, n) => (n.length > m ? n.length : m),
    0,
  );
  ...
  if (pickEnv("LEGACY_ENV_WARN", "1") !== "0") {
    const presetLegacy = [...__ENV_PRESET_KEYS__]
    ...
    if (presetLegacy.length) {
      ...
      const shown = presetLegacy
        ...
        .join(" ");
      const more =
        presetLegacy.length > 12 ? ` …(+${presetLegacy.length - 12} more)` : "";
  ...
  if (doctorOn) {
    const defFileDisp = DEFENDER_SOURCE_FILE
      ? path.basename(DEFENDER_SOURCE_FILE)
      : "(unknown)";
    console.log(
    ...
    if (legacyReportAliasUsed) {
      console.log(
        `CONFIG NOTE: LEGACY_REPORT is acting as LEGACY_OUTPUT=${outputMode}.`,
      );
    }
    if (legacyProgressRaw) {
      console.log(
        `CONFIG NOTE: LEGACY_PROGRESS=${legacyProgressRaw} is ignored by legacy-sim.`,
      );
    }
  }
  ...
  if (headerMode !== "min") {
    const defLine =
      headerMode === "full"
        ? defenderNames.join(", ")
        : summarizeList(defenderNames, defListMax);
  ...
  const defsSig = hashStr32(
    stableStringify(
      payloads.map((p) => ({ name: p.name, payload: p.payload })),
    ),
  );
  ...
  for (let vi = 0; vi < variants.length; vi++) {
    ...
    const cfg = {
      ...
      armorStatStack:
        v.armorStatStack && v.armorStatStack !== "inherit"
          ? v.armorStatStack
          : "sum4",
      armorStatRound:
        v.armorStatRound && v.armorStatRound !== "inherit"
          ? v.armorStatRound
          : v.statRound,
      armorStatSlots:
      ...
      crystalSlots: Number.isFinite(Number(v.crystalSlots))
        ? Number(v.crystalSlots)
        : 4,
    ...
    const tactTag =
      cfg.tacticsMode === "none"
        ? "none"
        : `${cfg.tacticsMode}:${cfg.tacticsVal}`;
    const armSlotsTag =
      cfg.armorStatSlots === "inherit" ? "" : `/s${cfg.armorStatSlots}`;
    const armStatTag =
    ...
    const runKey = hashStr32(
      stableStringify({ logicKey, trials, seed, deterministic, rngMode }),
    );
    ...
    if (printBaselineTemplate) {
      ...
      console.log(
        "--- BASELINE TEMPLATE (paste/merge into legacy-baselines.json) ---",
      );
    ...
    function partCrystal(part) {
      ...
      if (typeof part.upgrades === "string" && part.upgrades)
    ...
    for (let di = 0; di < payloads.length; di++) {
      ...
      if (deterministic) {
        ...
        const s = mix32(
          (seed ^ mix32((logicKey ^ Math.imul(di, 0x9e3779b9) ^ tag) | 0)) | 0,
        );
        RNG =
          rngMode === "fast"
      ...
      const defender = compileCombatantFromParts({
        ...
        w1V: (() => {
          ...
          return getV(
            payload.weapon1.name,
            partCrystal(payload.weapon1),
            u1,
            u2,
          );
        })(),
        w2V: (() => {
          ...
          return getV(
            payload.weapon2.name,
            partCrystal(payload.weapon2),
            u1,
            u2,
          );
        })(),
      ...
      if (debugStats) {
        const aArmorName =
          (ATTACKER_BUILD &&
            ATTACKER_BUILD.armor &&
            ATTACKER_BUILD.armor.name) ||
          "";
        const aArmorCr =
          (ATTACKER_BUILD &&
            ATTACKER_BUILD.armor &&
            ATTACKER_BUILD.armor.crystal) ||
          "";
        const dArmorName =
          (payload && payload.armor && payload.armor.name) || "";
        const dArmorCr =
          (payload && payload.armor ? partCrystal(payload.armor) : "") || "";
        console.log(
          `  DBG: ` +
            ...
            (aArmorName
              ? `A_armor=${aArmorName}${aArmorCr ? ` [${aArmorCr}]` : ""} | `
              : "") +
            (dArmorName
              ? `D_armor=${dArmorName}${dArmorCr ? ` [${dArmorCr}]` : ""} | `
              : "") +
      ...
      const shouldTrace =
        traceFights > 0 && (traceDef ? traceDef === name : di === 0);
      ...
      const gbFile = compareBaselines
        ? baselineLookup(baselineStore, baselineKeyHex, name)
        : null;
      ...
      {
        const firstDisp =
          aFirstLabel === "yes" ? "A" : aFirstLabel === "no" ? "D" : "Rand";
  ...
  ${lineDetails}`;
        ...
        if (!verboseBlocks && sepOn && di < payloads.length - 1)
          console.log(sepLine);
        ...
        if (verboseBlocks) {
          ...
          if (PRINT_WEAPON_RNG) {
            const fmtR = (c) =>
              Number.isFinite(c.rngMin) && c.dmg > 0
                ? `${c.rngMin}-${c.rngMax}`
                : "n/a";
            ...
            console.log(
              `  RNG by weapon: A_w1=${Aw1} A_w2=${Aw2} | D_w1=${Dw1} D_w2=${Dw2}`,
            );
          }
          ...
          if (debugDps) {
            ...
            const A_dpa = stats.A.acts
              ? stats.A.totalActionDmg / stats.A.acts
              : 0;
            const D_dpa = stats.D.acts
              ? stats.D.totalActionDmg / stats.D.acts
              : 0;
            console.log(
          ...
          if (gb) {
            ...
            if (cfg.diagArmor) {
              ...
              try {
                const meta =
                  defender && defender.__meta ? defender.__meta : null;
                ...
                if (
                  meta &&
                  delta !== 0 &&
                  (meta.w1?.itemName === "Void Sword" ||
                    meta.w2?.itemName === "Void Sword")
                ) {
                  ...
                  let bestPost = applyWeaponDmgRound(
                    baseNow * (1 + dmgPctSum),
                    cfg.weaponDmgRound,
                  );
                  let bestErr = Math.abs(bestPost - needPost);
                  for (
                    let b = Math.max(1, baseNow - 25);
                    b <= baseNow + 25;
                    b++
                  ) {
                    const post = applyWeaponDmgRound(
                      b * (1 + dmgPctSum),
                      cfg.weaponDmgRound,
                    );
  ...
  if (compareBaselines && variantCmpSummaries.length > 1) {
    ...
    for (const k of keys) {
      const s = new Set();
      for (const row of variantCmpSummaries)
        s.add(String((row.params || {})[k]));
