 ...
 * v2.12.3 (CATALOG CONFIRM SPEEDUP + mixed bonus clarified)
 * =====================
 *
 * Fixes / Improvements:
...
/** Crystal / upgrade / rounding config (must match legacy-sim-latest.js) */
function normalizeStatRoundMode(s, fallback = 'ceil') {
  const v = String(s ?? '')
    .trim()
    .toLowerCase();
  if (v === 'ceil' || v === 'floor' || v === 'round') return v;
  return fallback;
}
function normalizeArmorStatStackMode(s) {
  const v = String(s ?? '')
    .trim()
    .toLowerCase();
  if (v === 'inherit' || v === 'sum4' || v === 'iter4') return v;
  return 'inherit';
}
function normalizeArmorStatRoundMode(s) {
  const v = String(s ?? '')
    .trim()
    .toLowerCase();
  if (v === 'inherit' || v === 'ceil' || v === 'floor' || v === 'round') return v;
  return 'inherit';
}
function normalizeArmorStatSlotsMode(s) {
  const v = String(s ?? '')
    .trim()
    .toLowerCase();
  if (v === 'inherit') return 'inherit';
  const n = Number(v);
  return Number.isFinite(n) ? Math.max(0, Math.floor(n)) : 'inherit';
}

const LEGACY_STAT_ROUND = normalizeStatRoundMode(_envStr('LEGACY_STAT_ROUND', 'ceil'));
const LEGACY_WEAPON_DMG_ROUND = normalizeStatRoundMode(_envStr('LEGACY_WEAPON_DMG_ROUND', 'ceil'));
const LEGACY_CRYSTAL_SLOTS = Math.max(
  0,
  Math.floor(Number(_envStr('LEGACY_CRYSTAL_SLOTS', '4')) || 4),
);

const LEGACY_CRYSTAL_STACK_MODE = normalizeCrystalStackMode(
  _envStr('LEGACY_CRYSTAL_STACK_MODE', 'sum4'),
);
const LEGACY_CRYSTAL_STACK_STATS = normalizeCrystalStackMode(
  _envStr('LEGACY_CRYSTAL_STACK_STATS', LEGACY_CRYSTAL_STACK_MODE),
);
const LEGACY_CRYSTAL_STACK_DMG = normalizeCrystalStackMode(
  _envStr('LEGACY_CRYSTAL_STACK_DMG', LEGACY_CRYSTAL_STACK_MODE),
);

const LEGACY_ARMORSTAT_STACK = normalizeArmorStatStackMode(
  _envStr('LEGACY_ARMORSTAT_STACK', 'inherit'),
);
const LEGACY_ARMORSTAT_ROUND = normalizeArmorStatRoundMode(
  _envStr('LEGACY_ARMORSTAT_ROUND', 'inherit'),
);
const LEGACY_ARMORSTAT_SLOTS = normalizeArmorStatSlotsMode(
  _envStr('LEGACY_ARMORSTAT_SLOTS', 'inherit'),
);

/** Misc "no crystal skill" config (must match legacy-sim-latest.js) */
const MISC_NO_CRYSTAL_SKILL = new Set(
  String(_envStr('LEGACY_MISC_NO_CRYSTAL_SKILL', ''))
    .split(',')
    .map((x) => x.trim())
    .filter(Boolean),
);
const MISC_NO_CRYSTAL_SKILL_TYPE_MULTS = parseSkillTypeMultipliers(
  _envStr('LEGACY_MISC_NO_CRYSTAL_SKILL_TYPES', ''),
);
const MISC_NO_CRYSTAL_SKILL_SLOT2_TYPE_MULTS = parseSkillTypeMultipliers(
  _envStr('LEGACY_MISC_NO_CRYSTAL_SKILL_SLOT2_TYPES', ''),
);
const MISC_SLOT2_MULTS_ACTIVE =
  MISC_NO_CRYSTAL_SKILL_SLOT2_TYPE_MULTS && MISC_NO_CRYSTAL_SKILL_SLOT2_TYPE_MULTS.size > 0;
...
/** Small baseline tweak overrides (match legacy-sim-latest.js defaults) */
(() => {
  const HF_ARMOR_BASE_OVERRIDE = Number(_envStr('LEGACY_HF_ARMOR_BASE_OVERRIDE', '125'));
  if (Number.isFinite(HF_ARMOR_BASE_OVERRIDE) && ItemDefs['Hellforged Armor']?.flatStats) {
    ItemDefs['Hellforged Armor'].flatStats.armor = HF_ARMOR_BASE_OVERRIDE;
  }
  const VOID_SWORD_BASE_MIN_OVERRIDE = _envStr('LEGACY_VOID_SWORD_BASE_MIN_OVERRIDE', '').trim();
  const VOID_SWORD_BASE_MAX_OVERRIDE = Number(
    _envStr('LEGACY_VOID_SWORD_BASE_MAX_OVERRIDE', '123'),
  );
  if (ItemDefs['Void Sword']?.baseWeaponDamage) {
    const b = ItemDefs['Void Sword'].baseWeaponDamage;
    if (VOID_SWORD_BASE_MIN_OVERRIDE !== '') {
      const n = Number(VOID_SWORD_BASE_MIN_OVERRIDE);
      if (Number.isFinite(n)) b.min = n;
    }
    if (Number.isFinite(VOID_SWORD_BASE_MAX_OVERRIDE)) b.max = VOID_SWORD_BASE_MAX_OVERRIDE;
  }
})();
...
const DEFENDER_ALIAS_TO_CANON = new Map([
  ['SG1 Split bombs', 'SG1 Split Bombs T2'],
...
function resolveDefenderKey(raw) {
  ...
  // If this name is a known alias *and* the canonical key exists, always prefer canonical.
  // This prevents aliases from inflating defender counts and double-weighting a defender.
  const canon = DEFENDER_ALIAS_TO_CANON.get(name);
  if (canon && DEFENDER_PAYLOADS && DEFENDER_PAYLOADS[canon]) return canon;

  // Exact match (canonical or otherwise).
...
function priorityFirstAll(mode) {
  ...
  const push = (raw) => {
    ...
    if (!key) {
      if (isMainThread) console.warn(`WARN: defender "${raw}" not found; skipping`);
      return;
    }
    if (!seen.has(key)) {
      seen.add(key);
...
function DEFENDER_NAMES() {
  ...
  for (const it of items) {
    ...
    if (!key) {
      if (isMainThread) console.warn(`WARN: defender "${it}" not found; skipping`);
      continue;
    }
    if (!seen.has(key)) {
      seen.add(key);
...
// =====================
function computeVariant(itemName, crystalName, upgrades = [], slotTag = 0) {
  // slotTag currently unused in brute-sim (misc slots are treated as orderless pairs),
  // but kept for parity with legacy-sim configuration.
  const idef = ItemDefs[itemName];
  ...
  for (const u of upgrades) {
    ...
    if (!udef) throw new Error(`Unknown upgrade: ${u}`);
    udefs.push(udef);
  }

  // Validate upgrades against the item, if the defs specify slots.
  if (idef.upgradeSlots && idef.upgradeSlots.length > 0) {
    for (const u of udefs) {
      if (!idef.upgradeSlots.includes(u.name)) {
        throw new Error(`Upgrade "${u.name}" not allowed for item "${itemName}"`);
      }
    }
  }
  ...
  const crystalPct0 = cdef.pct ? cdef.pct : null;
  const crystalPct = crystalPct0 ? { ...crystalPct0 } : null;

  // Apply misc "no crystal skill" reductions, matching legacy-sim-latest.
  // NOTE: brute-sim treats misc slots as ORDERLESS (i<=j) pairs. Slot2-specific multipliers require
  // ordered slots; we warn in main() if LEGACY_MISC_NO_CRYSTAL_SKILL_SLOT2_TYPES is set.
  if (
    crystalPct &&
    MISC_NO_CRYSTAL_SKILL.has(itemName) &&
    MISC_NO_CRYSTAL_SKILL_TYPE_MULTS.size > 0
  ) {
    for (const k of ['gun', 'melee', 'proj', 'def']) {
      const p = crystalPct[k] || 0;
      if (!p) continue;
      const mult = MISC_NO_CRYSTAL_SKILL_TYPE_MULTS.get(k);
      if (mult === undefined) continue;
      crystalPct[k] = p * mult;
    }
  }

  // Combine upgrade pct maps (stats + damage).
  const upgradePct = {};
  for (const u of udefs) {
    const pct = u.pct || {};
    for (const k of Object.keys(pct)) {
      upgradePct[k] = (upgradePct[k] || 0) + (pct[k] || 0);
    }
  }

  const crystalSlots = LEGACY_CRYSTAL_SLOTS;
  const statRound = LEGACY_STAT_ROUND;
  const wepDmgRound = LEGACY_WEAPON_DMG_ROUND;

  const stackModeStats = LEGACY_CRYSTAL_STACK_STATS;
  const stackModeDmg = LEGACY_CRYSTAL_STACK_DMG;

  const stackModeArmor =
    LEGACY_ARMORSTAT_STACK === 'inherit' ? stackModeStats : LEGACY_ARMORSTAT_STACK;
  const roundModeArmor = LEGACY_ARMORSTAT_ROUND === 'inherit' ? statRound : LEGACY_ARMORSTAT_ROUND;
  const slotsArmor =
    LEGACY_ARMORSTAT_SLOTS === 'inherit' ? crystalSlots : LEGACY_ARMORSTAT_SLOTS || crystalSlots;

  const fs = idef.flatStats || {};
  const stat = (base, key, allowArmorSpecial = false) => {
    const p = crystalPct ? crystalPct[key] || 0 : 0;
    const slots = allowArmorSpecial ? slotsArmor : crystalSlots;
    const stack = allowArmorSpecial ? stackModeArmor : stackModeStats;
    const roundMode = allowArmorSpecial ? roundModeArmor : statRound;

    let x = base;
    if (p) x = applyCrystalPctToStat(x, p, slots, roundMode, stack);
    const up = upgradePct[key] || 0;
    if (up) x = x + roundStat(x * up, statRound);
    return x;
  };

  const addSpeed = stat(fs.speed || 0, 'speed');
  const addArmStat = stat(fs.armor || 0, 'armor', true); // armor uses armorStat config
  const addAcc = stat(fs.accuracy || 0, 'accuracy');
  const addDod = stat(fs.dodge || 0, 'dodge');
  const addGun = stat(fs.gunSkill || 0, 'gun');
  const addMel = stat(fs.meleeSkill || 0, 'melee');
  const addPrj = stat(fs.projSkill || 0, 'proj');
  const addDef = stat(fs.defSkill || 0, 'def');

  let weapon = null;
  let weaponMaskBit = 0;

  if (idef.baseWeaponDamage) {
    const bd = idef.baseWeaponDamage;
    const dmgPct = crystalPct ? crystalPct.damage || 0 : 0;

    let min = bd.min;
    let max = bd.max;

    if (dmgPct) {
      const w = applyCrystalPctToWeaponDmg(
        min,
        max,
        dmgPct,
        crystalSlots,
    ...
    if (upDmg) {
      min = roundWeaponDmg(min * (1 + upDmg), wepDmgRound);
      max = roundWeaponDmg(max * (1 + upDmg), wepDmgRound);
    }

    weapon = { name: itemName, min, max, skill: idef.weaponSkill };

    if (idef.weaponSkill === 'gun') weaponMaskBit = WEAPON_MASK.GUN;
    else if (idef.weaponSkill === 'melee') weaponMaskBit = WEAPON_MASK.MELEE;
    else if (idef.weaponSkill === 'proj') weaponMaskBit = WEAPON_MASK.PROJ;
  }

  const cl = CrystalShort[crystalName] || crystalName[0] || '?';
  let tag = `${cl}`;

  if (udefs.length) {
    const uTag = udefs.map((u) => u.short || u.name[0] || '?').join('+');
    tag += `{${uTag}}`;
  }
  ...
  return {
    ...
    weapon,
    weaponMaskBit,
    __misc: MISC_NAMES.has(itemName),
  };
}
...
// =====================
async function main() {
  // NOTE: These legacy-sim knobs are not modeled in the brute sim yet.
  // If you have them exported in your shell, you'll get mismatched results vs legacy-sim.
  if (process.env.LEGACY_MISC_NO_CRYSTAL_SKILL || process.env.LEGACY_MISC_NO_CRYSTAL_SKILL_TYPES) {
    console.warn(
      'WARN: LEGACY_MISC_NO_CRYSTAL_* is set but is currently ignored by brute-sim (results may differ vs legacy-sim).',
    );
  }
  ...
  // By default, Stage 0/Stage A early-bail uses the raw screen margin (LEGACY_SCREEN_MARGIN).
  // If you want the bailing to be as forgiving as the catalog capture band, enable:
  //   LEGACY_ALIGN_BAIL_TO_CATALOG=1
  const alignBailRaw = String(process.env.LEGACY_ALIGN_BAIL_TO_CATALOG ?? '')
    .trim()
    .toLowerCase();
  ...
  const effectiveBailMargin = alignBailToCatalog
  ...
  if (initFloor !== null) {
    ...
    if (MISC_SLOT2_MULTS_ACTIVE) {
      console.log(
        'WARN: LEGACY_MISC_NO_CRYSTAL_SKILL_SLOT2_TYPES is set, but brute-sim currently treats misc slots as orderless pairs; slot2-specific multipliers are ignored.',
      );
    }
  }
  ...
  console.log(
    `LEGACY brute-force (v2.12.3) | defenders=${defenderBuilds.length} | trialsGate/def=${TRIALS_GATE} | trialsScreen/def=${TRIALS_SCREEN} | trialsConfirm/def=${TRIALS_CONFIRM} | hidden=${HIDDEN_PRESET}`,
  );
  console.log(
    `Workers=${workers} (logical=${logical}, physicalGuess=${physicalGuess}) | RNG=${
      rngMode === 'fast' ? 'fast(sfc32)' : 'Math.random'
    } | deterministic=${deterministic ? 'ON' : 'OFF'} | tactics=${TACTICS_MODE}:${TACTICS_BASE_VAL}`,
  );
  console.log(
  ...
  console.log(
    `Gatekeepers: N=${GATEKEEPERS} trialsGate=${TRIALS_GATE} | bail margin=${effectiveBailMargin.toFixed(
      2,
    )}% (raw=${BAIL_MARGIN.toFixed(2)}%, catalog=${catalogMargin.toFixed(2)}%, align=${
      alignBailToCatalog ? 'ON' : 'OFF'
    })`,
  );
  console.log(
    ...
    for (let w = 0; w < workers; w++) {
      ...
      const wk = new Worker(__filename, {
        workerData: {
          ...
          gateBailMargin: effectiveBailMargin,
          screenBailMargin: effectiveBailMargin,
          maxTurns: SETTINGS.MAX_TURNS,
